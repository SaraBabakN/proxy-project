...
# normalize = transforms.Normalize(mean=[0.4914, 0.4822, 0.4465],std=[0.2473, 0.2434, 0.2610])
...
class cifarDataProvider(DataProvider):
    ...
    def __init__(self, save_path='./datasets', train_batch_size=64, test_batch_size=200, valid_size=0.1,
                drop_last=True, use_cutout=True, cutout_n_holes=1, cutout_size=16,
                 n_worker=2, resize_scale=0.08, distort_color=None, **kwargs):
      ...
      if valid_size is not None:
        ...
        train_indexes, valid_indexes = self.random_sample_valid_set(
                 train_set.targets, valid_size, self.n_classes)

        train_sampler = torch.utils.data.sampler.SubsetRandomSampler(train_indexes)
        valid_sampler = torch.utils.data.sampler.SubsetRandomSampler(valid_indexes)

        self.train = torch.utils.data.DataLoader(train_set, batch_size=train_batch_size,
                                                  sampler=train_sampler,
                                                  pin_memory=torch.cuda.is_available(),
                                                  num_workers=n_worker,
                                                  drop_last=drop_last)
        self.valid = torch.utils.data.DataLoader(valid_set, batch_size=test_batch_size,
      ...
      self.test = torch.utils.data.DataLoader(test_set, batch_size=test_batch_size, 
                                                shuffle=False,
                                                pin_memory=torch.cuda.is_available(), 
                                                num_workers=n_worker,
                                                drop_last=False)
